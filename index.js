...
// создаем таблицу и инпут
const table = document.createElement("table");
const input = document.createElement("input");
// textForChange - вспомогательная переменная, которая хранит саму ячейку на которую кликнули, чтобы иизменить контент
let textForChange;
// dataCounter - вспомогательная переменная для отображения пользоватей на странице
let dataCounter = 0;

// вешаем обработчик события на инпут, чтоб при изменении в самом инпуте отобразить контент в ячейке
input.addEventListener("change", (e) => {
  textForChange.target.textContent = e.target.value;
});
...
// вспомогательная переменная которую будем отображать
let filteredData = [];

// заполняем переменную для отображения первых 10 пользователей
for (dataCounter; dataCounter < 10; dataCounter++) {
  filteredData.push(data[dataCounter]);
}

// функция, которая создает разметку и добавляет ее на страницу по передаваемому массиву
const displayContent = (arr) => {
  arr.map((el) => {
    const content = `<tr><td class='firstName'>${el.name.firstName}</td><td class='lastName'>${el.name.lastName}</td><td class="about">${el.about}</td><td class='eyeColor'> <div style='margin: 0 auto;width: 2rem; height: 2rem; border-radius: 5px; background-color: ${el.eyeColor}'></div></td></tr>`;
    table.insertAdjacentHTML("beforeend", content);
  });
};
// создаем кнопки, селект и чекбоксы
const createHelpers = () => {
  ...
  // функция, которая прячет колонку при выборе чекбокса(получаем id чекбокса, получаем все ячейки с таким же классом и меням класс, который скрывает/отображает)
  const hideColumn = (column) => {
    [...document.getElementsByClassName(column.target.id)].forEach((el) =>
      el.classList.toggle("show")
    );
  };

  // добавляем разметку в селект
  sortSelect.insertAdjacentHTML(
    "afterbegin",
    "<option disabled selected value='sort'>Sort</option><option value='firstName'>Имя</option><option value='lastName'>Фамилия</option><option value='about'>Описание</option><option value='eyeColor'>Цвет глаз</option>"
  );
  // добавляем разметку чекбоксов
  divCheckBox.insertAdjacentHTML(
    "afterbegin",
    '<input type="checkbox" class="check" id="firstName">Имя</input><input  class="check" type="checkbox" id="lastName">Фамилия</input><input  class="check" type="checkbox" id="about">Описание</input><input class="check" type="checkbox" id="eyeColor">Цвет глаз</input> '
  );
  // добавляем классы
  ...
  // добаляем текст в кнопки
  prev.textContent = "Previous";
  next.textContent = "Next";

  // добавляем обработчики событий на чекбоксы и селектор
  divCheckBox.addEventListener("change", (e) => hideColumn(e));
  sortSelect.addEventListener("change", (e) => {
    sortColumn(e.target.value);
  });

  // добавляем обработчик события на кнопку next, обнуляем селектор и чекбоксы
  next.addEventListener("click", () => {
    // класс show содержит свойство display: none и, если при переключении страницы есть скрытая колонка, мы убираем этот класс
    if (document.getElementsByClassName("show").length > 0) {
      [...document.getElementsByClassName("check")].forEach(
        (el) => (el.checked = false)
      );
    ...
    // если счетчик больше чем размер данных - отобразить алерт
    if (dataCounter === data.length) {
      alert("data ended");
      return;
    }
    // обнуляем вспомогательный массив и заполняем его следующими данными
    filteredData = [];
    for (let i = dataCounter; i < dataCounter + 10; i++) {
  ...
  prev.addEventListener("click", () => {
    ...
    filteredData = [];
    for (let i = dataCounter - 20; i < dataCounter - 10; i++) {
      filteredData.push(data[i]);
    }
  ...
  // добавляем кнопки, чекбоксы и селект в рутовый компонент
  buttonsContainer.appendChild(prev);
  buttonsContainer.appendChild(next);
  div.appendChild(divCheckBox);
  div.appendChild(sortSelect);
  div.appendChild(buttonsContainer);
  main.insertAdjacentElement("afterbegin", div);
};

// функция сортировки, получаем идентификатор и сортируем колонку в алфавитном порядке функцией sort
const sortColumn = (columnName) => {
  if (columnName === "firstName" || columnName === "lastName") {
    filteredData.sort((a, b) => {
      if (a.name[columnName].toUpperCase() > b.name[columnName].toUpperCase()) {
        return 1;
      }
      if (b.name[columnName].toUpperCase() > a.name[columnName].toUpperCase()) {
        return -1;
      }
      return 0;
    });
    ...
    filteredData.sort((a, b) => {
      ...
      if (b[columnName].toUpperCase() > a[columnName].toUpperCase()) {
        return -1;
      }
      return 0;
    });
...
// функция удаления контента, удалет все колонки кроме заглавных
const deleteContent = () => {
  filteredData.forEach(() => table.removeChild(table.lastChild));
};

// функция переключаения класса у инпута для редактировния контента
const toggleInput = () => {
  input.classList.toggle("show");
};

// функция отображения инпута для редактирования контента, при нажатии на ячейку берем ее координаты, записываем сам элемент в textContent
// добавляем инпут справа от ячейки
const showInputForChange = (element) => {
  toggleInput();
  textForChange = element;
  input.style.top = `${element.target.getBoundingClientRect().y + 10}px`;
  input.style.left = `${element.target.getBoundingClientRect().right}px`;
  input.value = element.target.innerHTML;
  console.log(element.target.parentNode);
};

// функция клика, если нажали на ячейку не заглавную, отобразить справа от нее инпут
const clickHandler = (e) => {
  if (e.target.nodeName === "TD") {
    showInputForChange(e);
  }
};

// добавляем обработчики на инпут
input.addEventListener("focusout", toggleInput);
// при клике на таблицу ивент всплывет и мы поймем на какую ячейку нажали
table.addEventListener("click", clickHandler);

// добавляем заглавные ячейки в таблицу
table.insertAdjacentHTML(
  "afterbegin",
  "<table><tr><th class='firstName'>Имя</th><th class='lastName'>Фамилия</th><th class='about'>Описание</th><th class='eyeColor'>Цвет глаз</th></tr></table>"
);

// добавляем таблицу и инпут в рутовый компонет
main.appendChild(table);
main.appendChild(input);
// создаем кнопки, инпут и чекбоксы
createHelpers();

// отображаем первых 10 пользователей
displayContent(filteredData);
